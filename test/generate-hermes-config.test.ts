...
describe("agents/hermes/generate-config.ts", () => {
  ...
  it.each([
    {
      ...
      expectedEnv: "ANTHROPIC_API_KEY=openshell:resolve:env:ANTHROPIC_API_KEY",
      forbiddenEnv: "OPENAI_API_KEY=openshell:resolve:env:OPENAI_API_KEY",
    },
    {
      providerKey: "openai",
      expectedProvider: "openai",
      expectedEnv: "OPENAI_API_KEY=openshell:resolve:env:OPENAI_API_KEY",
      forbiddenEnv: "ANTHROPIC_API_KEY=openshell:resolve:env:ANTHROPIC_API_KEY",
    },
    {
  ...
  ])("maps provider key $providerKey for Hermes runtime config", (tc) => {
    const { config, envFile } = runConfigScript({
    ...
    if (tc.expectedEnv) {
      expect(envFile).toContain(`${tc.expectedEnv}\n`);
    }
    expect(envFile).not.toContain(`${tc.forbiddenEnv}\n`);
  });

  it("configures Hermes managed tools through the NemoClaw host broker", () => {
    const { config, envFile } = runConfigScript({
    ...
    expect(envFile).toMatch(/^API_SERVER_KEY=[a-f0-9]{64}$/m);
    ...
    expect(envFile).not.toContain("NOUS_REFRESH_TOKEN");
    expect(envFile).not.toContain("NOUS_ACCESS_TOKEN");
  });
